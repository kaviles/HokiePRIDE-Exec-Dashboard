...
function removePatron($patronData) {
    ...
    if ($mysqli) {
        ...
        $qs = $mysqli->prepare("SELECT email, itemcount, status FROM library_patrons WHERE email = ?");
        ...
        if ($qs_num_rows == 1) {
            ...
            if ($r_status == 'ADDED') {
                ...
                if ($r_itemcount === 0) {
                    ...
                    $qu = $mysqli->prepare("UPDATE library_patrons SET status=?, status_by=?, status_timestamp=?, 
